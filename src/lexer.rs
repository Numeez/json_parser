...
impl Lexer {
            ...
            Some(b'\\')=>{
                ...
                match self.current(){
                    ...
                    Some(b'u')=>{
                        ...
                        let ch = char::from_u32(codepoint).unwrap_or_else(|| panic!("Invalid codepoint: {}",codepoint));
                        result.push(ch);
                    }
            ...
            match self.current(){
                Some(c)=>{
                    if c==*expected{
                        ...
                        panic!("Unexpected character: {} while parsing value: {}", c as char, keyword)
                    }
                },
                None=>{
    ...
    pub fn read_number(&mut self)->f64{
        ...
        value.parse::<f64>().unwrap_or_else(|_| panic!("Invalid number:{}",value))
    }
}
